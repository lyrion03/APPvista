...
public sealed class ProcessSnapshotProvider : IProcessSnapshotProvider
{
            ...
            foreach (var process in Process.GetProcesses())
            {
                using (process)
                {
                    ...
                    try
                    {
                        ...
                        var executablePath = lightweight ? string.Empty : TryGetExecutablePath(process);
                        ...
                        var cpuUsage = lightweight ? 0d : GetCpuUsage(process, now, out cpuAvailable);
                        var memoryUsageAvailable = lightweight
                            ? TryGetWorkingSetFast(process, out var memoryUsage)
                            : TryGetMemoryUsage(process, out memoryUsage);
                        var ioUsage = lightweight ? IoUsage.Empty : GetIoUsage(process, now, out ioAvailable);
                        var hasMainWindow = HasMainWindow(process);
                        var threadCount = lightweight ? 0 : TryGetThreadCount(process, out threadCountAvailable);
    ...
    private static bool TryGetWorkingSetFast(Process process, out MemoryUsage memoryUsage)
    {
        memoryUsage = default;
        ...
        try
        {
            ...
            memoryUsage = new MemoryUsage(workingSetBytes, 0, 0);
            return true;
        }
        catch
        {
